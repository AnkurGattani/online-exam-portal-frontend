...
import { AssignQuestionService, AssignedQuestionToExamDTO }   from '../../services/assign-question/assign-question-service';
@Component({
  selector: 'app-assign-question',
  standalone: true,
  imports: [
    CommonModule,FormsModule, MatCardModule, MatFormFieldModule,
    MatInputModule, MatButtonModule, MatSelectModule, MatSnackBarModule, MatIconModule,
...
export class AssignQuestionComponent {
  ...
  constructor(private assignService: AssignQuestionService) {}
...
formSubmit() {
    const examId = 1; // Replace dynamically
    this.assignService.assignQuestions(examId, this.criteriaList).subscribe({
      next: () => {
        this.assignService.getAssignedQuestions(examId).subscribe({
          next: (data) => {
    ...
    console.log('Calling:', `${this.baseUrl}/${examId}/assign-questions`);
  }
